...
The externally reachable companion surface is one token-protected, rate-limited route with a fixed source and bounded `lines` parameter. It filters to warning/error severity, limits response bytes, disables caching, and redacts common secret shapes. Regex redaction is incomplete by nature and is not the security boundary. Network isolation, authentication, fixed routing, bounds, and minimization are primary.
