...
GPT Actions call `POST /api/v1/services/call` with `entity_id` as an array and a structured `data` object containing the parameters returned by Home Assistant service discovery. This accommodates all Home Assistant service parameter shapes without exposing a transparent proxy. When Home Assistant marks a service as response-only, the gateway automatically adds the REST `return_response` query flag and returns the service response to the Action. For a multi-step request, such as mode + temperature + fan mode on climate equipment, use `POST /api/v1/services/batch`; every item must name explicit allowed entities and the batch stops on the first Home Assistant error. A batch item can target several compatible entities with the same service data.
